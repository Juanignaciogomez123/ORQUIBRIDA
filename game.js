document.addEventListener('DOMContentLoaded', () => {
  ...
  const backToGalleryGameOverBtn = document.getElementById('back-to-gallery-game-over'); // Nuevo botón

  const gridSize = 20;
  let snake, food, direction, score, gameOver, gameLoop, extraLives, isMovingFoodActive, isPaused;
  ...
  function placeFood() {
    ...
    const powerUpKeys = Object.keys(powerUpTypes); // Obtener todas las claves de powerUpTypes
    const foodType = powerUpKeys[Math.floor(Math.random() * powerUpKeys.length)]; // Seleccionar aleatoriamente
    const foodImageSrc = powerUpTypes[foodType];
    ...
    // Solo activar el movimiento si el tipo de comida es MOVING_FOOD
    if (foodType === 'MOVING_FOOD') {
  ...
  function update() {
    if (gameOver || isPaused) return; // No actualizar si el juego ha terminado o está pausado
  ...
  function changeDirection(event) {
    ...
    if (!isPaused) { // Solo permitir cambio de dirección si el juego no está pausado
    ...
    if (key === 'p') { // Tecla 'P' para pausar/reanudar
      togglePause();
    }
  }
  ...
  function endGame() {
    ...
    pauseMenu.classList.add('hidden'); // Ocultar menú de pausa
    clearInterval(gameLoop);
    if (foodMoveInterval) clearInterval(foodMoveInterval);

    finalScoreElement.textContent = `Puntuación Final: ${score}`; // Mostrar el puntaje final
  ...
  function togglePause() {
    ...
    if (isPaused) {
      ...
      gameLoop = setInterval(update, 150);
      if (isMovingFoodActive) foodMoveInterval = setInterval(moveFood, 20);
  ...
  function startGame() {
    ...
    pauseMenu.classList.add('hidden'); // Asegurarse de que el menú de pausa esté oculto al inicio
    ...
    extraLives = 0;
    gameOver = false;
    isPaused = false; // Asegurarse de que no esté pausado al inicio
    isMovingFoodActive = false;
    scoreElement.textContent = `Puntuación: ${score}`;
    livesElement.textContent = `Vidas Extra: ${extraLives}`;
    placeFood();
  ...
  backToGalleryGameOverBtn.addEventListener('click', () => {
